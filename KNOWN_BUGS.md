...
## Lower Priority
- **Donation status not refreshed on PayPal success**: Currently relies on `refreshMe()` on login. If user donates without logout, `hasDonated` won't update until next session. Mitigation: Call `refreshMe()` in PaymentDialog after successful donation record.
