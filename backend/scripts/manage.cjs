...
if (!action) {
  console.error('Usage: node backend/scripts/manage.cjs <start|stop|status> [--foreground]');
  process.exit(2);
}

const root = path.resolve(__dirname, '..', '..');

// Minimal .env loader to read HOST_IP/GOL_HOST without extra deps
...
function start() {
  ...
  try {
    ...
    waitForPort(configuredHost, portNum, 20000)
      .then(() => console.log(`backend appears to be accepting connections on ${configuredHost}:${waitPort}`))
      .catch(() => {
        if (configuredHost !== localFallback) {
          return waitForPort(localFallback, portNum, 8000)
            .then(() => console.log(`backend appears to be accepting connections on ${localFallback}:${waitPort} (fallback)`))
            .catch((err) => console.warn(`backend port check failed on ${configuredHost} and localhost: ${err.message}`));
        }
        console.warn('backend port check failed: timeout');
      });
  } catch (e) {
