...
function start() {
  ...
  if (existing && isRunning(existing)) {
    console.log(
      `backend already running (pid=${existing}). Logs: ${config.logFile}`,
    );
    process.exit(0);
  }
  ...
  try {
    ...
    if (child.pid) {
      ...
      if (shouldDetach)
        console.log(
          `backend started in background (pid=${child.pid}). Logs: ${config.logFile}`,
        );
      else console.log(`backend started in foreground (pid=${child.pid}).`);
    } else {
      console.log(
        `backend started (no pid available). Logs: ${config.logFile}`,
      );
    }
    ...
    waitForPort(host, Number.parseInt(waitPort, 10), 20000)
      .then(() =>
        console.log(
          `backend appears to be accepting connections on ${host}:${waitPort}`,
        ),
      )
      .catch((err) =>
        console.warn(`backend port check failed: ${err.message}`),
      );
  } catch (e) {
