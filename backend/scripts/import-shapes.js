...
const main = async () => {
  ...
  console.log(
    dry ? "Dry run: no file will be written" : "Merging and writing to dest",
  );
  ...
  console.log(
    `Found ${srcData.length} shapes in source; ${dstData.length - added} already present; ${added} to add.`,
  );
  if (added > 0 && !dry) {
