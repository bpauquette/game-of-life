...
async function testParsing() {
  ...
  try {
    ...
    patterns.slice(0, 5).forEach((pattern, i) => {
      ...
      console.log(
        `   Size: ${pattern.width}x${pattern.height} (${pattern.cellCount} cells)`,
      );
      console.log(
        `   Cells: ${JSON.stringify(pattern.cells.slice(0, 3))}${pattern.cells.length > 3 ? "..." : ""}`,
      );
      console.log("");
    });
    ...
    for (const name of notable) {
      ...
      if (pattern) {
        console.log(
          `${pattern.name}: ${pattern.cellCount} cells, ${pattern.width}x${pattern.height}`,
        );
      }
    }
  } catch (error) {
