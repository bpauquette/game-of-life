...
async function parseLexicon() {
      ...
      if (currentPattern && patternLines.length > 0) {
        try {
          ...
          if (hasRLE) {
            ...
            if (rleStart >= 0) {
              ...
              try {
                ...
                console.warn(`Failed to parse RLE for ${currentPattern.name}:`, e.message);
              }
            }
          } else {
            // Dot-asterisk format - look for lines with dots and asterisks
            const dotLines = patternLines.filter(l => 
              l.includes('.') && l.includes('*') && 
              l.startsWith('\t') // Patterns are indented
          ...
          if (cells.length > 0) {
            ...
            console.log(`Found pattern: ${currentPattern.name} (${cells.length} cells)`);
          }
        } catch (e) {
          console.warn(`Error processing pattern ${currentPattern?.name}:`, e.message);
        }
      }
  ...
  if (currentPattern && patternLines.length > 0) {
    try {
      ...
      console.warn(`Error processing final pattern ${currentPattern?.name}:`, e.message);
    }
  }
