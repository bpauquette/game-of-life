...
export function AuthProvider({ children }) {
  ...
  const refreshMe = async () => {
    try {
      ...
      if (data?.hasSupported !== undefined) setHasSupported(!!data.hasSupported);
  ...
  const login = (email, token) => {
    ...
    // Refresh current user info to populate hasSupported
    refreshMe();
  };
  ...
  return (
    <AuthContext.Provider value={{ token, email, hasSupported, login, logout, refreshMe }}>
      {children}
