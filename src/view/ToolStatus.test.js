...
describe('ToolStatus', () => {
  test('shows cursor when no tool-specific state', () => {
    render(<ToolStatus selectedTool="draw" toolStateRef={{ current: {} }} cursorCell={{ x: 5, y: 7 }} />);
  ...
  test('formats line tool start/end/delta', () => {
    ...
    render(<ToolStatus selectedTool="line" toolStateRef={ref} cursorCell={null} />);
  ...
  test('handles missing start gracefully', () => {
    ...
    render(<ToolStatus selectedTool="line" toolStateRef={ref} cursorCell={{ x: 3, y: 3 }} />);
