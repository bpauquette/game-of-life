...
describe("useCanvasManager", () => {
  ...
  describe("mouse event handlers", () => {
    it("should handle mouse events without throwing", () => {
      ...
      expect(
        actFn(() => result.current.handleMouseDown(mockEvent)),
      ).not.toThrow();

      expect(
        actFn(() => result.current.handleMouseMove(mockEvent)),
      ).not.toThrow();

      expect(
        actFn(() => result.current.handleMouseUp(mockEvent)),
      ).not.toThrow();
    });
    ...
    it("should handle canvas click without throwing", () => {
      ...
      expect(
        actFn(() => result.current.handleCanvasClick(mockEvent)),
      ).not.toThrow();
    });
  });
  ...
  describe("error handling", () => {
    it("should handle invalid color scheme gracefully", () => {
      ...
      const { result } = renderHook(() =>
        useCanvasManager(propsWithBadColorScheme),
      );
    ...
    it("should handle missing tool gracefully", () => {
      ...
      const { result } = renderHook(() =>
        useCanvasManager(propsWithMissingTool),
      );
    ...
    it("should log errors when overlay drawing fails", () => {
      const mockPropsWithFailingTool = {
        ...
        toolMap: {
          draw: {
            drawOverlay: jest.fn(() => {
              throw new Error("Mock overlay error");
            }),
          },
        },
      };
      ...
      const { result } = renderHook(() =>
        useCanvasManager(mockPropsWithFailingTool),
      );
      ...
      expect(mockProps.logger.warn).toHaveBeenCalledWith(
