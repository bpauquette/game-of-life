...
import {
  ...
  drawLiveCells,
} from "../../controller/utils/canvasUtils";
...
export const useCanvasManager = ({
  ...
  const draw = useCallback(() => {
    ...
    const computedOffset = computeComputedOffset(
      canvasRef.current,
      offsetRef,
      cellSize,
    );
  ...
  const drawShapePreview = useCallback(
    (ctx, selShape, last, computedOffset) => {
      ...
      for (const element of cells) {
        ...
        try {
          ctx.fillStyle =
            colorScheme?.getCellColor?.(last.x + cx, last.y + cy) ?? "#222";
        } catch (err) {
          logger.warn(err);
          ctx.fillStyle = "#222";
        }
  ...
  const drawToolOverlay = useCallback(
    (ctx, computedOffset) => {
  ...
  const resizeCanvas = useCallback(() => {
    ...
    const logicalHeight =
      globalThis.window?.innerHeight || DEFAULT_WINDOW_HEIGHT;
  ...
  const handleShapeToolMove = useCallback(
    (e, tool, pt) => {
      if (!pt) return;
  ...
  const shouldToolMove = useCallback((e) => {
    return (
      e.buttons & 1 || toolStateRef.current.last || toolStateRef.current.start
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    (e) => {
      if (isPanningRef.current) {
        ...
        try {
          ...
          /* releasePointerCapture not supported */
        }
        return;
      }
