...
const GameOfLifeApp = () => {
  ...
  const [popWindowSize, setPopWindowSize] = useState(
    DEFAULT_POPULATION_WINDOW_SIZE,
  );
  const [popTolerance, setPopTolerance] = useState(
    DEFAULT_POPULATION_TOLERANCE,
  );
  const [steadyInfo, setSteadyInfo] = useState({
    steady: false,
    period: 0,
    popChanging: false,
  });
  ...
  const updateStabilityDetection = useCallback((game) => {
    ...
    const popSteady = isPopulationStable(
      populationHistory,
      popWindowSizeRef.current,
      popToleranceRef.current,
    );
  ...
  useEffect(() => {
    if (gameRef.current && colorScheme) {
      ...
      gameRef.current.view.renderer.updateOptions({
        backgroundColor:
          colorScheme.background || colorScheme.backgroundColor || "#000000",
  ...
  // Shape management
  const { recentShapes, selectShape, selectShapeAndClosePalette } =
    useShapeManager({
      selectedShape,
      setSelectedShape,
      selectedTool,
  ...
  const handleLoadGrid = useCallback((liveCells) => {
    ...
    try {
      ...
      logger.info(
        `Loaded grid with ${liveCells ? liveCells.size : 0} live cells`,
      );
    } catch (error) {
  ...
  const getViewport = useCallback(() => {
    return gameRef.current
      ? gameRef.current.getViewport()
      : { offsetX: 0, offsetY: 0, cellSize: 8 };
  }, []);
  ...
  return (
    <div className="canvas-container">
      ...
      <ControlsBar
        ...
        popHistoryRef={{
          current: gameRef.current?.getPopulationHistory() || [],
        }}
      ...
      {uiState.paletteOpen && (
        <ShapePaletteDialog
          ...
          backendBase={
            process.env.REACT_APP_BACKEND_BASE || "http://localhost:55000"
          }
          colorScheme={
            colorSchemes ? colorSchemes[uiState.colorSchemeKey] || {} : {}
          }
        />
      )}
      ...
      <DebugConsole isVisible={true} maxLines={50} />
    </div>
  );
};
