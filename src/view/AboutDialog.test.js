...
describe("AboutDialog", () => {
  test("renders about dialog when open", () => {
    ...
    expect(
      screen.getByText("Interactive Cellular Automaton Simulator"),
    ).toBeInTheDocument();
  });
  ...
  test("does not render when closed", () => {
    ...
    expect(screen.queryAllByText("About Conway's Game of Life")).toHaveLength(
      0,
    );
  });
  ...
  test("contains links to external resources", () => {
    ...
    expect(githubLink).toHaveAttribute(
      "href",
      "https://github.com/bpauquette/game-of-life",
    );
    expect(githubLink).toHaveAttribute("target", "_blank");

    const lifewikiLink = screen.getByRole("link", {
      name: "LifeWiki - Pattern Database",
    });
