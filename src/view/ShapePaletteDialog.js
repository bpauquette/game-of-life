...
export default function ShapePaletteDialog({
  open,
  onClose,
  onSelectShape,
  backendBase,
  ...
  const retryBackendConnection = async () => {
    ...
    try {
      ...
      if (isHealthy) {
        ...
        setBackendError(
          "Backend server is still not responding. Please make sure it's running on port 55000.",
        );
      }
    } catch (error) {
      logger.warn("Backend health check failed:", error.message);
      setBackendError(
        "Failed to connect to backend. Please ensure it's running.",
      );
    } finally {
      setBackendStarting(false);
    }
  ...
  return (
    <>
        ...
        <DialogContent>
              ...
              <ListItem key={`${s.id || "shape"}-${idx}`} disablePadding>
                <ListItemButton
                  onClick={async () => {
                    ...
                    try {
                      const base = getBaseUrl(backendBase);
                      const url = new URL(
                        `/v1/shapes/${encodeURIComponent(s.id)}`,
                        base,
                      );
                      const res = await fetch(url.toString());
                      if (res.ok) {
                      ...
                      logger.warn(
                        "Failed to fetch full shape data, using metadata only:",
                        err,
                      );
                ...
                >
                  <ListItemText
                    ...
                    secondary={`${s.width}×${s.height} — ${s.cellsCount || 0} cells`}
                  />
                  <Box
                    sx={{
                      ml: 1,
                  ...
                  >
                    <svg
                      ...
                      viewBox={`0 0 ${Math.max(1, s.width || 1)} ${Math.max(1, s.height || 1)}`}
                      preserveAspectRatio="xMidYMid meet"
                      style={{
                        background: colorScheme.background || "transparent",
                        border: `1px solid rgba(0,0,0,${PREVIEW_BORDER_OPACITY})`,
                    ...
                    >
                      {Array.isArray(s.cells) && s.cells.length > 0 ? (
                        s.cells.map((c) => (
                          <rect
                            key={`${s.id || "shape"}-${c.x},${c.y}`}
                            x={c.x}
                            y={c.y}
                            width={1}
                            height={1}
                            fill={getCellColor(c.x, c.y)}
                          />
                        ))
                      ) : (
                        // small empty placeholder grid
                        <g
                          stroke={`rgba(0,0,0,${PREVIEW_BORDER_OPACITY})`}
                          fill="none"
                        >
                          {Array.from(
                            { length: Math.max(1, s.width || 1) },
                            (_, i) => (
                              <line
                                key={`v-${s.id || "unknown"}-${i}`}
                                x1={i + GRID_LINE_OFFSET}
                                y1={0}
                                x2={i + GRID_LINE_OFFSET}
                                y2={Math.max(1, s.height || 1)}
                              />
                            ),
                          )}
                          {Array.from(
                            { length: Math.max(1, s.height || 1) },
                            (_, j) => (
                              <line
                                key={`h-${s.id || "unknown"}-${j}`}
                                x1={0}
                                y1={j + GRID_LINE_OFFSET}
                                x2={Math.max(1, s.width || 1)}
                                y2={j + GRID_LINE_OFFSET}
                              />
                            ),
                          )}
                        </g>
          ...
          >
            <div>
              {total > 0 && (
                <small>
                  {total} shapes in catalog
                  {total > LARGE_CATALOG_THRESHOLD
                    ? " — large catalog, use search or paging"
                    : ""}
                </small>
              )}
            </div>
          ...
          <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
            ...
            <DialogContent>
              ...
              <strong>
                {toDelete?.name || toDelete?.meta?.name || "this shape"}
              </strong>
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    const base = getBaseUrl(backendBase);
                    const url = new URL(
                      `/v1/shapes/${encodeURIComponent(id)}`,
                      base,
                    );
                    const res = await fetch(url.toString(), {
                      method: "DELETE",
      ...
      >
        ...
        <DialogContent>
          <Typography sx={{ mb: 2 }}>
            The shapes catalog backend server doesn't appear to be running. You
            need to start it to access the shapes catalog.
          </Typography>
          {backendError && (
          ...
          <Typography variant="body2" color="text.secondary">
            The backend provides access to a catalog of Conway's Game of Life
            patterns and shapes.
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            startIcon={
              backendStarting ? <CircularProgress size={16} /> : <UndoIcon />
            }
          >
