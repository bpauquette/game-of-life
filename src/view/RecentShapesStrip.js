...
const SELECTED_BOX_SHADOW =
  "0 0 10px rgba(0, 255, 136, 0.6), inset 0 0 5px rgba(0, 255, 136, 0.2)";
...
const RecentShapesStrip = ({
  ...
  const getShapeDimensions = (cells) => {
    if (cells.length === 0)
      return { width: DEFAULT_SHAPE_SIZE, height: DEFAULT_SHAPE_SIZE };
  ...
  const isShapeSelected = (shape) => {
    ...
    if (shape.name && selectedShape.name)
  ...
  const renderShapeCells = (cells, shapeKey, colorScheme) => {
    return cells.map((cell, index) => {
      ...
      const fillColor =
        colorScheme?.getCellColor?.(x, y) ?? DEFAULT_SHAPE_COLOR;
          ...
          <button
            ...
            onMouseEnter={(e) => {
              ...
              if (!isShapeSelected(shape)) {
                e.currentTarget.querySelector("svg").style.borderColor =
          ...
          >
            <svg
              ...
              style={{
                ...
                borderRadius: SHAPE_BORDER_RADIUS,
                boxShadow: isShapeSelected(shape)
                  ? SELECTED_BOX_SHADOW
                  : "none",
