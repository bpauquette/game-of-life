...
function start() {
  ...
  if (existing && isRunning(existing)) {
    console.log(
      `${area} already running (pid=${existing}). Logs: ${config.logFile}`,
    );
    process.exit(0);
  }
  ...
  const foreground =
    ...
    process.argv.includes("--no-detach");
  ...
  try {
    if (shouldDetach) {
      ...
      try {
        ...
        console.warn(
          "Unable to open log file for writing, falling back to ignore:",
          e.message,
        );
        outFd = "ignore";
      }
      ...
      child = spawn(spawnCmd, spawnArgs, {
        ...
        env: {
          ...process.env,
          ...(area === "backend" ? { PORT: config.portEnv } : {}),
        },
      });
      ...
      if (
        ...
        config.cmd &&
        config.cmd.toLowerCase().indexOf("npm") !== -1
      ) {
  ...
  try {
    ...
    if (child.pid) {
      ...
      if (shouldDetach)
        console.log(
          `${area} started in background (pid=${child.pid}). Logs: ${config.logFile}`,
        );
      else console.log(`${area} started in foreground (pid=${child.pid}).`);
    } else {
      console.log(
        `${area} started (no pid available). Logs: ${config.logFile}`,
      );
    }
    ...
    const waitPort =
      area === "frontend"
        ...
        : process.env.GOL_BACKEND_PORT || process.env.PORT || config.portEnv;
    const host = "127.0.0.1";
    waitForPort(host, Number.parseInt(waitPort, 10), 20000)
      .then(() =>
        console.log(
          `${area} appears to be accepting connections on ${host}:${waitPort}`,
        ),
      )
      .catch((err) =>
        console.warn(`${area} port check failed: ${err.message}`),
      );
  } catch (e) {
